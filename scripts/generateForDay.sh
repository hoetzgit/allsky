...
if [[ ${DO_KEOGRAM} == "true" ]]; then
	...
	if [[ ${TYPE} == "GENERATE" ]]; then
		CMD="'${ALLSKY_BIN}/keogram' ${SIZE_FILTER} -d '${DATE_DIR_CLEAN}' -e ${EXTENSION} -o '${UPLOAD_FILE}' ${KEOGRAM_EXTRA_PARAMETERS}"
...
if [[ ${DO_STARTRAILS} == "true" ]]; then
	...
	if [[ ${TYPE} == "GENERATE" ]]; then
		CMD="'${ALLSKY_BIN}/startrails' ${SIZE_FILTER} -d '${DATE_DIR_COMPASS}' -e ${EXTENSION} -b ${BRIGHTNESS_THRESHOLD} -o '${UPLOAD_FILE}' ${STARTRAILS_EXTRA_PARAMETERS}"
