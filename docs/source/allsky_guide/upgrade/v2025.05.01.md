...
## System Page / Scripts Module { data-toc-label="Custom Sscript" }

For security reasons scripts used in Buttons on the system page or run via the scripts module can only be run from the following directory

```
~/allsky/config/myFile
```

If you have scripts being run via either of these systems in any other location please move them into the myFiles directory.